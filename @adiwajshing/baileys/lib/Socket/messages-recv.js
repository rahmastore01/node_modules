...
const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, onUnexpectedError, assertSessions, assertingPreKeys, sendNode, relayMessage, sendReceipt, resyncMainAppState, emitEventsFromMap, } = sock;
    ...
    const processMessageLocal = async (message) => {
        var _a, _b;
        const meId = authState.creds.me.id;
        // send ack for history message
        const isAnyHistoryMsg = !!message.message && !!((_b = (_a = Utils_1.normalizeMessageContent(message.message)) === null || _a === void 0 ? void 0 : _a.protocolMessage) === null || _b === void 0 ? void 0 : _b.historySyncNotification);
        if (isAnyHistoryMsg) {
            await sendNode({
                tag: 'receipt',
                attrs: {
        ...
        const newEvents = await process_message_1.default(message, { historyCache, meId, keyStore: authState.keys, logger, treatCiphertextMessagesAsReal });
        if (isAnyHistoryMsg) {
    ...
    const handleNotification = async (node) => {
        ...
        await processingMutex.mutex(remoteJid, () => {
